...
sdp_session_t *register_service(uint8_t rfcomm_channel) {
	...
	*/
	...
	printf("==PrinTwogether== Operating.. \n");
...
int init_server() {
	...
	printf("Waiting for bluetooth calling..\n");
	client = accept(sock, (struct sockaddr *)&rem_addr, &opt);
	//printf("accept() returned %d\n", client);

	ba2str(&rem_addr.rc_bdaddr, buffer);
	//fprintf(stderr, "accepted connection from %s\n", buffer);
	printf("accepted connection from Phone\n");
...
int main()
{
	...
	mbtowc(0, 0, 0); // reset
	char buf[1024]; // 
	int cho[3], jung[3], jong[3]; // 초중종성 숫자 넣을공간
	int k;
	while (1) {
	...
	if (recv_message == NULL) {
		...
		wc_len = mbtowc(&wc, buf + buf_offset, buf_len - buf_offset);
		while (wc_len  > 0) {
...
//		printf("%d",k);
		printf("A");
		//return 0;
		system ("./i2ctest/main 1");
	
	}
	//int ret = system ("./i2ctest/main 1");
